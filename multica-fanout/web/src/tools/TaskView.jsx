...
function NodeBox({ x, y, width, children, tone = 'default', onClick }) {
  const tones = {
  ...
  return (
    <div
      className={`absolute -translate-x-1/2 -translate-y-1/2 cursor-pointer rounded-xl border px-3 py-2.5 shadow-sm transition-transform hover:scale-[1.03] ${tones[tone]}`}
...
export default function TaskView() {
  ...
  const [agentDrawer, setAgentDrawer] = useState(null); // { id, data } 抽屉中的 agent
  ...
  useEffect(() => {
    ...
    let alive = true;
    const fetchDetail = () =>
      runToolWithPath('task-monitor', 'task', activeId)
        .then((d) => alive && (setDetail(d), setDetailError(null)))
        .catch((e) => alive && setDetailError(e.message));
    fetchDetail();
    const timer = setInterval(fetchDetail, 6000); // 6s 自动刷新
    return () => {
      alive = false;
      clearInterval(timer);
    };
    ...
    <div className="mx-auto max-w-[1240px] px-8 py-8">
      <header className="mb-6">
        ...
        <p className="mt-1.5 text-sm leading-relaxed text-slate-500 max-w-[65ch]">
          一次派发 = 一个任务：从起点出发，多个 Agent 并行工作，最终汇总结果。点击任一 Agent 节点查看其实时工作情况。
        </p>
      </header>
      ...
      <div className="relative rounded-2xl border border-slate-200/60 bg-white shadow-sm">
        {detailError && !detail ? (
          <div className="p-8">
            <ErrorState message={detailError} onRetry={() => setActiveId(activeId)} />
          </div>
        ) : !detail ? (
          ...
          <div className="relative h-[440px]">
            <svg className="absolute inset-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
              ...
              {detail.children.map((c, i) => {
                const y = nodeY(i, detail.children.length);
                return (
                  <g key={c.id}>
                    <path d={`M 18 ${50} L 34 ${y}`} fill="none" stroke={c.status === 'done' ? '#10b981' : '#cbd5e1'} strokeWidth="0.7" markerEnd="url(#arrow)" />
                    <path d={`M 68 ${y} L 82 ${50}`} fill="none" stroke={c.status === 'done' ? '#10b981' : '#cbd5e1'} strokeWidth="0.7" markerEnd="url(#arrow)" />
                  </g>
                );
              })}
            </svg>
            ...
            <NodeBox x={4} y={50} width={13} tone="start">
              ...
              <div className="truncate text-[10px] text-slate-300">{detail.parent.title.replace(/^\[并行\] /, '')}</div>
            </NodeBox>
            ...
            {detail.children.map((c, i) => {
              ...
              return (
                <NodeBox key={c.id} x={51} y={y} width={16} tone={tone} onClick={() => openAgent(c.agentId)}>
            ...
            {/* 汇点：聚合 */}
            <NodeBox x={86} y={50} width={13} tone="sink">
              <div className="truncate text-[10px] font-medium uppercase tracking-wide text-slate-400">汇总</div>
              <div className="mt-0.5 text-xs font-semibold text-slate-800">
                {detail.counts.done}/{detail.counts.total}
              </div>
              <div className="mt-1 flex gap-1">
                <Badge tone="ok">{detail.counts.done} done</Badge>
                {detail.counts.running > 0 && <Badge tone="accent">{detail.counts.running} 运行中</Badge>}
              </div>
            </NodeBox>
          </div>
        )}
        ...
        {detail && (
          <div className="flex flex-wrap items-center gap-4 border-t border-slate-100 px-5 py-2.5 text-[11px] text-slate-500">
            ...
            <span className="flex items-center gap-1.5"><StatusDot tone="muted" /> 离线</span>
            <span className="ml-auto">自动每 6s 刷新 · 点击 Agent 节点查看实时工作</span>
          </div>
        )}
      </div>
      ...
      {agentDrawer && (
        <div className="fixed inset-0 z-40" role="dialog" aria-modal="true">
          ...
          <aside className="absolute right-0 top-0 flex h-full w-[400px] max-w-[92vw] flex-col border-l border-slate-200 bg-white shadow-2xl">
